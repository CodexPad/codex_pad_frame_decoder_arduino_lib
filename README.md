...
This library and example code are designed for scenarios where **CodexPad gamepad data is received via a serial Bluetooth transparent transmission module**. The following two connection approaches are supported:
...
- **Development board with an external Bluetooth transparent transmission module**
    ...
    Suitable for development boards that **do not have an integrated Bluetooth transparent transmission module** (such as Arduino UNO, Nano, Mega 2560, etc.). An external serial Bluetooth transparent transmission module (e.g., NL‑16) is used to receive and parse the data frames from the CodexPad gamepad.
